...
VecSimIndex *NewIndex(const TieredIndexParams *params) {
    ...
    } else if (params->primaryIndexParams->algo == VecSimAlgo_CUVS_IVFFLAT ||
               params->primaryIndexParams->algo == VecSimAlgo_CUVS_IVFPQ) {
#ifdef USE_CUDA
        return TieredCuvsIvfFactory::NewIndex(params);
#else
...
size_t EstimateInitialSize(const TieredIndexParams *params) {
    ...
    if (params->primaryIndexParams->algo == VecSimAlgo_HNSWLIB) {
        est += TieredHNSWFactory::EstimateInitialSize(params, bf_params);
               ...
               params->primaryIndexParams->algo == VecSimAlgo_CUVS_IVFPQ) {
#ifdef USE_CUDA
        est += TieredCuvsIvfFactory::EstimateInitialSize(params);
#else
        throw std::runtime_error("CUVS_IVFFLAT and CUVS_IVFPQ are not supported in CPU version");
#endif
    }
...
size_t EstimateElementSize(const TieredIndexParams *params) {
    ...
    if (params->primaryIndexParams->algo == VecSimAlgo_HNSWLIB) {
        est = HNSWFactory::EstimateElementSize(&params->primaryIndexParams->algoParams.hnswParams);
               ...
               params->primaryIndexParams->algo == VecSimAlgo_CUVS_IVFPQ) {
#ifdef USE_CUDA
        est = CuvsIvfFactory::EstimateElementSize(
            &params->primaryIndexParams->algoParams.cuvsIvfParams);
#else
        throw std::runtime_error("CUVS_IVFFLAT and CUVS_IVFPQ are not supported in CPU version");
#endif
    }
