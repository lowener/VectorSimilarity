...
TieredRaftCAGRAParams createDefaultCAGRATieredParams(size_t dim) {
    RaftCAGRAParams params = {.dim = dim,
                              .metric = VecSimMetric_L2};
    TieredRaftCAGRAParams params_tiered = {.CAGRAParams = params};
    return params_tiered;
}
...
TYPED_TEST(RaftIvfTieredTest, RaftIVFTiered_CAGRA_add_sanity_test) {
    size_t dim = 4;
    ...
    std::shared_ptr<VecSimAllocator> allocator = VecSimAllocator::newVecsimAllocator();
    TieredRaftCAGRAParams params_tiered = createDefaultCAGRATieredParams(dim);
    params_tiered.tieredParams = tiered_params;

    VecSimIndex *index = RaftCAGRAFactory::NewTieredIndex(&params_tiered, allocator);
    VecSimQueryParams queryParams = {.batchSize = 1};
